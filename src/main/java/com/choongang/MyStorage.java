...
public class MyStorage {
    ...
    static void prod_input(Scanner s){
        ...
        for (int i =0; i < products.length; i++ ){
            if (products[i].equals("등록 가능")) {
                productIdx = i;}
            if (productIdx != -1){
                products[productIdx] =inPutProduct;
    ...
    static void prod_remove(Scanner s){
        ...
        Scanner scanner = new Scanner(System.in);
        String outPutProduct = scanner.nextLine();
        for (int i =0; i < products.length; i++ ) {
            if (products[i].equals(outPutProduct)) {
                products[i] = "등록 가능";
                counts[i] = 0;
            }
        }}
...
//        int cnt = Integer.parseInt(s.nextLine());  // 수량 입력 받기
    ...
    public static void prod_amount_decrease(Scanner s) {
        ...
        String outGoods = s.nextLine();
        System.out.println("[System] 원하는 출고량을 입력하세요 :");
        String digit = "1234567890";
        String str = s.nextLine();
        // if str이 숫자로 변경 불가능하면 return;
        if (digit.indexOf(str)==-1){
            return;
        }else {
        // 아니면 아래 코드 실행되도록
        int outGoodsNum = Integer.parseInt(str);
        for (int i =0; i < products.length; i++ ){
            if (products[i].equals(outGoods)) {
                if (counts[i] < outGoodsNum){
                    break;
                }
                counts[i] = counts[i] - outGoodsNum;
            }
        }
        System.out.println("[Clear] 출고가 완료되었습니다.");
    }}

//                if (digit.indexOf(outGoodsNum)==-1 ){
